...
type Unique<T, U extends any[] = []> = T extends [infer First, ...infer Rest]
...
type GameInProgressBoard = [Row, Row, Row];
...
type GetCoords<T extends TicTacToePositions> =
  T extends `${infer Vertical extends TicTacToeYPositions}-${infer Horizontal extends TicTacToeXPositions}`
    ? { y: CoordsMap[Vertical]; x: CoordsMap[Horizontal] }
    : never;
...
> = CurrentRow extends [
  ...
  ? Memory["length"] extends X
    ? PlaceChipAtX<Rest, X, Chip, [...Memory, Chip]>
    : PlaceChipAtX<Rest, X, Chip, [...Memory, First]>
  : Memory;

type PlaceChipAtPosition<
  Board extends GameInProgressBoard,
...
> = Memory["length"] extends Board["length"]
  ...
  ? PlaceChipAtPosition<
      ...
      Chip,
      Coords,
      [...Memory, PlaceChipAtX<Board[Memory["length"]], Coords["x"], Chip>]
    >
  : PlaceChipAtPosition<
      Board,
      Positions,
      Chip,
      Coords,
      [...Memory, Board[Memory["length"]]]
    >;

type CanPlaceChipAtPosition<
  Board extends GameInProgressBoard,
  Positions extends TicTacToePositions,
  Coords extends GetCoords<Positions> = GetCoords<Positions>
> = Board[Coords["y"]][Coords["x"]] extends TicTacToeEmptyCell ? true : false;

type CanPlaceChipAtPositionTest1 = CanPlaceChipAtPosition<
  TestBoard4,
  "bottom-left"
>;
type CanPlaceChipAtPositionTest2 = CanPlaceChipAtPosition<
  TestBoard5,
  "bottom-right"
>;

type FlipGameState<CurrentState extends TicTacToeChip> =
...
type CheckRowsWin<Board> = Board extends [
  infer First extends TicTacToeCell[],
  ...infer Rest extends TicTacToeCell[][]
]
  ? First[number] extends "  "
    ? CheckRowsWin<Rest>
    : Unique<First>["length"] extends 1
    ? `${First[0]} Won`
    : CheckRowsWin<Rest>
  : void;

type CheckRowsForDraw<Board> = Board extends [
  infer First extends TicTacToeCell[],
  ...infer Rest extends TicTacToeCell[][]
]
...
type CheckForWin<CurrentGame> = CurrentGame extends string[][]
  ? CheckRowsWin<Transpose<CurrentGame>>
  : never;
...
type TicTacToe<
  ...
  Positions extends TicTacToePositions,
  Result extends PlaceChipAtPosition<
    CurrentGame["board"],
    Positions,
    CurrentGame["state"]
